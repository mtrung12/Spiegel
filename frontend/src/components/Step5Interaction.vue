                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  <div class="loading-icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <circle cx="12" cy="12" r="10" stroke-width="4" stroke="#E5E5E5"></circle>
                      <path d="M12 2a10 10 0 0 1 10 10" stroke-width="4" stroke="#4D4D4D" stroke-linecap="round"></path>
                    </svg>
                  </div>
...
const chatHistoryCache = ref({}) // Every conversation: { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
const selectReportAgentChat = () => {
  ...
  // Restore the report agent's conversation
...
const selectAgent = (agent, idx) => {
  ...
  // Restore that agent's conversation
...
const renderMarkdown = (content) => {
  ...
  // Handle lists, nested ones included
...
const sendMessage = async () => {
  ...
  try {
    ...
    scrollToBottom()
    // Save the conversation into the cache
    saveChatHistory()
  }
}
...
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    // The data lives at res.data.result.results, which is an object map
    // Shape: {"twitter_0": {...}, "reddit_0": {...}}, or one platform only
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // Flatten the map to an array, preferring the Reddit reply
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Prefer Reddit, fall back to Twitter
...
/* Left panel - report styling, identical to Step4Report.vue */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: 1px solid #F5F5F5; padding-bottom: 8px; }
...
/* Ordered-list numbering: a CSS counter keeps several <ol>s in sequence */
.message-text {
...
/* Quote styling in the chat and questionnaire areas */
