  ...
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <!-- Notice shown once the simulation finishes -->
...
const showEdgeLabels = ref(true) // Edge labels are shown by default
const expandedSelfLoops = ref(new Set()) // Expanded self-loop entries
const showSimulationFinishedHint = ref(false) // Notice shown once the simulation finishes
const wasSimulating = ref(false) // Whether a simulation was running a moment ago

// Dismiss the finished notice
const dismissFinishedHint = () => {
  showSimulationFinishedHint.value = false
}

// Watch isSimulating to detect the end of a run
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Running to not-running: show the finished notice
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Toggle a self-loop entry open or closed
...
// Collect the entity types for the legend
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // Reset what is expanded
}
...
const renderGraph = () => {
  ...
  // Stop the previous force simulation
  ...
  // Process the edges, counting and indexing the edges between each node pair
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loop edges, grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count the edges per node pair and collect the self-loops
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // A self-loop: collect it
  ...
  // Which edge of each pair we are up to
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Self-loop nodes already handled
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop: each node gets a single merged loop
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Detail for every self-loop edge
        }
      })
      return
    }
    ...
    // Does the edge run the same way as the canonical direction (source UUID < target UUID)?
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Curvature: several edges fan out, a lone edge stays straight
    let curvature = 0
    if (totalCount > 1) {
      // Spread the curvature evenly so the edges stay distinct
      // The range widens with the edge count
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // An edge running against the canonical direction has its curvature flipped,
      // so every edge is laid out in the same frame and none overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Force simulation - node spacing scales with the edge count
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Distance scales with how many edges join this pair
      // 150 base, plus 40 per additional edge
    ...
    // Pull toward the centre, so isolated clusters gather in the middle
  ...
  // Links - drawn as paths, so they can curve
  ...
  const getLinkPath = (d) => {
    ...
    // Self-loop?
    if (d.isSelfLoop) {
      // Self-loop: an arc leaving the node and coming back
      const loopRadius = 30
      // Leaves to the right and loops around
      const x1 = sx + 8  // Start offset
      const y1 = sy - 4
      const x2 = sx + 8  // End offset
      const y2 = sy + 4
      // Draw the loop as an arc (sweep-flag=1 is clockwise)
    ...
    // Control point, scaled by the edge count and the distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Offset perpendicular to the link, proportional to the distance so the curve stays visible.
    // More edges means a larger proportion.
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // 25% base, plus 5% per additional edge
  ...
  // Midpoint of the curve, used to place the label
  const getLinkMidpoint = (d) => {
    ...
    // Self-loop?
    if (d.isSelfLoop) {
      // Self-loop label sits to the right of the node
      return { x: sx + 70, y: sy }
    }
    ...
    // Midpoint of the quadratic Bezier, t=0.5
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    const pairTotal = d.pairTotal || 1
    ...
    // Quadratic Bezier: B(t) = (1-t)^2 P0 + 2(1-t)t P1 + t^2 P2, at t=0.5
  ...
  // Link label background - white, so the text stays legible
  ...
  // Keep the refs so visibility can be toggled from outside
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record the position only; do not restart the simulation (this may be a click, not a drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Has a real drag started? (moved past the threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart the simulation only once a real drag is confirmed
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only a real drag lets the simulation cool down
        if (d._isDragging) {
    ...
    .on('click', (event, d) => {
      ...
      // Highlight the edges attached to it
  ...
  simulation.on('tick', () => {
    // Update the curve paths
    link.attr('d', d => getLinkPath(d))
    
    // Update the edge label positions; unrotated text reads better
    linkLabels.each(function(d) {
      const mid = getLinkMidpoint(d)
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // Drop the rotation and stay horizontal
    })
    
    // Update the edge label backgrounds
    linkLabelBg.each(function(d, i) {
      ...
      d3.select(this)
        ...
        .attr('transform', '') // Drop the rotation
    })
  ...
  // A click on empty canvas closes the detail panel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
/* Styling for the simulation-finished notice */
