          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <!-- Comment visibility: how many replies reach one persona per
                     post, drawn by how the crowd reacted to them -->
                <div class="config-item">
                  <span class="config-item-label">{{ $t('step2.commentsSeenPerPost') }}</span>
                  <span class="config-item-value" :title="$t('step2.commentsSeenPerPostHint')">
                    {{ commentsSeenPerPost }}
                  </span>
                </div>
              </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Round count - shown only once the config is generated and the count is known.
               The slider is the control, present from the moment the number
               exists. It used to sit behind a toggle switch and a one-line
               "customise this" link, so the only setting on this screen was
               invisible until the reader found a hint they had no reason to
               look for. Audience size above is set exactly this way. -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            ...
            <div class="rounds-content custom">
              <div class="slider-display">
                <div class="slider-main-value">
                  <span class="val-num">{{ roundsValue }}</span>
                  <span class="val-unit">{{ $t('step2.roundsUnit') }}</span>
                  <!-- Says the slider is still where the generated config put
                       it, which is what the old "auto" card existed to say. -->
                  <span v-if="roundsAreAuto" class="auto-badge">{{ $t('step2.roundsAutoBadge') }}</span>
                </div>
                <div class="slider-meta-info">
                  <span>{{ $t('step2.estimatedDuration', { minutes: Math.round(roundsValue * 0.6) }) }}</span>
                </div>
              </div>
              ...
              <div class="range-wrapper">
                <input
                  ...
                  step="5"
                  class="minimal-slider"
                  :aria-label="$t('step2.roundsConfig')"
                  :style="{ '--percent': roundsPercent + '%' }"
                />
                <div class="range-marks">
                  <span>{{ MIN_ROUNDS }}</span>
                  <!-- Dropped when the generated count is itself 40: the mark
                       would land on top of the max label and claim the same
                       number twice. -->
                  <button
                    v-if="autoGeneratedRounds > RECOMMENDED_ROUNDS"
                    type="button"
                    class="mark-recommend"
                    :class="{ active: roundsValue === RECOMMENDED_ROUNDS }"
                    :disabled="readOnly"
                    @click="customMaxRounds = RECOMMENDED_ROUNDS"
                    :style="{ position: 'absolute', left: `calc(${recommendedPercent}% - 30px)` }"
                  >{{ $t('step2.recommendedRounds', { rounds: RECOMMENDED_ROUNDS }) }}</button>
                  <span>{{ autoGeneratedRounds }}</span>
                </div>
              </div>

              <p class="description">{{ $t('step2.roundsTip') }}</p>
            </div>
          </div>
...
// Round count. The slider rests on whatever the generated config works out to,
// so leaving it alone is the same choice the old "auto" card made - there is no
// separate mode, just a number that starts in the right place.
const MIN_ROUNDS = 10
const RECOMMENDED_ROUNDS = 40
// null until either the generated count lands or a saved choice arrives, so the
// two cannot race and 40 cannot become the answer by default.
...
watch(
  ...
  (saved) => {
    ...
    // `use_custom_rounds: false` means the run was set up on the generated
    // count, so whatever `max_rounds` sits beside it is the slider's old
    // resting position rather than a choice. Left null, the generated count
    // fills it in below.
    if (saved.use_custom_rounds === false) {
      customMaxRounds.value = null
    } else if (saved.max_rounds) {
      customMaxRounds.value = saved.max_rounds
    }
  },
  { immediate: true }
)
...
const autoGeneratedRounds = computed(() => {
  ...
  // Keep the cap at the recommended value or above, so the slider range stays sane
  return Math.max(calculatedRounds, RECOMMENDED_ROUNDS)
})

// The generated count is the slider's starting position, and its ceiling. This
// is also what fills the value in on first render: the config arrives long
// after this component mounts, and before it does there is no count to show.
watch(
  autoGeneratedRounds,
  (auto) => {
    if (!auto) return
    if (customMaxRounds.value === null) {
      customMaxRounds.value = auto
    } else {
      // A saved choice from a run set up when the config worked out longer
      // would otherwise sit past the end of the track.
      customMaxRounds.value = Math.min(Math.max(customMaxRounds.value, MIN_ROUNDS), auto)
    }
  },
  { immediate: true }
)

// Never null once the block is on screen - it only renders with a count - but
// the guard keeps the arithmetic below safe during the tick the config lands.
const roundsValue = computed(() => customMaxRounds.value ?? autoGeneratedRounds.value ?? RECOMMENDED_ROUNDS)

// Untouched means "run what the config worked out", which is what the old
// toggle recorded. Derived rather than stored: two sources for one fact only
// gave them a way to disagree.
const roundsAreAuto = computed(
  () => autoGeneratedRounds.value !== null && roundsValue.value === autoGeneratedRounds.value
)

const trackSpan = computed(() => Math.max((autoGeneratedRounds.value || MIN_ROUNDS) - MIN_ROUNDS, 1))
const roundsPercent = computed(() => ((roundsValue.value - MIN_ROUNDS) / trackSpan.value) * 100)
const recommendedPercent = computed(() => ((RECOMMENDED_ROUNDS - MIN_ROUNDS) / trackSpan.value) * 100)

// Comments a persona is shown under each post. Configs written before
// feed_config existed fall back to the backend default of 3; 0 means the
// persona sees the whole thread.
const commentsSeenPerPost = computed(() => {
  const configured = simulationConfig.value?.feed_config?.comments_per_post
  const perPost = configured === undefined || configured === null ? 3 : configured
  return perPost > 0 ? perPost : t('step2.commentsSeenAll')
})
...
const handleStartSimulation = async () => {
  ...
  await saveUserConfig({
    use_custom_rounds: !roundsAreAuto.value,
    max_rounds: roundsValue.value
  })

  // Build the payload for the parent
  const params = {}

  if (!roundsAreAuto.value) {
    // The slider was moved off the generated count: pass max_rounds
    params.maxRounds = roundsValue.value
    addLog(t('log.startSimCustomRounds', { rounds: roundsValue.value }))
  } else {
    // Still on the generated count: omit max_rounds and let the backend use it
...
/* The phase in hand carries a band travelling along its top edge. It sits on
   the card's own border line, so the five cards keep their exact positions
   whichever one is active. Inset by the corner radius rather than clipped with
   overflow, which would have cut off the popovers these cards contain. */
.step-card.active::before {
  ...
  animation: card-rail 2.8s linear infinite;
}
...
/* Same working chip as step 1: a light travelling through the fill, text
   contrast untouched. */
.badge.processing {
...
/* Marks the value as still the generated one. Quiet: it reports a state, it
   is not a control. */
.auto-badge {
  font-family: 'JetBrains Mono', monospace;
  font-size: 10px;
