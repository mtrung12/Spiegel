<template>
  <div class="process-page">
    <!-- Top navigation bar -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">CAMPAIGN REACTION</div>
    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graph visualisation, shown as soon as there is data -->
...
// The current project ID, which may change from 'new' to a real one
...
const currentPhase = ref(-1) // -1: uploading, 0: generating the ontology, 1: building the graph, 2: done
const selectedItem = ref(null) // The selected node or edge
...
const goToNextStep = () => {
  // TODO: move on to the environment setup step
  alert('环境搭建功能开发中...')
}
...
// Init: either create a new project or load an existing one
const initProject = async () => {
  const paramProjectId = route.params.projectId
  
  if (paramProjectId === 'new') {
    // New project: take the pending upload out of the store
...
// New project: call the ontology/generate API
const handleNewProject = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      // Update the project ID and data
      ...
      // Start the graph build automatically
...
// Start polling for graph data
const startGraphPolling = () => {
  // Fetch once immediately
  fetchGraphData()
  
  // Then fetch the graph data every 10 seconds
  graphPollTimer = setInterval(async () => {
    await fetchGraphData()
  }, 10000)
}

// Refresh the graph by hand
...
const fetchGraphData = async () => {
  try {
    // Read the project first, for its graph_id
    const projectResponse = await getProject(currentProjectId.value)
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        // Re-render only when the data changed
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // Show the progress as complete
        buildProgress.value = {
          progress: 100,
          message: '构建完成，正在加载图谱...'
        }
        
        // Reload the project to pick up graph_id
        const projectResponse = await getProject(currentProjectId.value)
        if (projectResponse.success) {
          projectData.value = projectResponse.data
...
const renderGraph = () => {
  ...
  // Map the nodes, so names can be looked up
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  // Collect the node IDs, so invalid edges can be filtered out
  ...
  // Draw the edges, each with a wide transparent line for hit-testing
  ...
  // The wide transparent line that catches clicks
  ...
  simulation.on('tick', () => {
    // Update every edge, the visible line and the hit area alike
    ...
    // Update the edge label positions
    linkLabel
...
/* Relationship view in the edge detail */
.edge-relation {
  display: flex;
  align-items: center;
  flex-wrap: wrap;
