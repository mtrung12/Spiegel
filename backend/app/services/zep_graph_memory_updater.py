...
Streams agent activity from a running simulation into the Zep graph.
"""
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Render the activity as text that can be sent to Zep.

        Uses a natural-language description so Zep can extract entities and
...
class ZepGraphMemoryUpdater:
    ...
    Zep graph. Activity is grouped per platform and flushed in batches of
    BATCH_SIZE.

    Every meaningful action is pushed to Zep; action_args carries the full
    context:
    ...
    def _send_batch_activities(
        ...
        *,
        ...
        """
        Flush a batch of activities into the Zep graph as one combined text.
...
class ZepGraphMemoryManager:
    """
    Registry of Zep graph memory updaters across simulations.
