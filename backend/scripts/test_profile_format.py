"""
Check that the generated profile formats match what OASIS expects.
Verifies:
...
def test_profile_formats():
    """Check the profile formats."""
    print("=" * 60)
    print("OASIS profile format check")
    print("=" * 60)
    
    # Build the test profile data
    test_profiles = [
        OasisAgentProfile(
            user_id=0,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if missing:
            ...
            print(f"\n   [PASS] every required field is present")
        
        # Check the Reddit JSON format
        print("\n2. Reddit profile (detailed JSON)")
        print("-" * 40)
        generator._save_reddit_json(test_profiles, reddit_path)
        
        # Read the JSON back and verify it
...
def show_expected_formats():
    """Print the formats OASIS expects."""
    print("\n" + "=" * 60)
    print("Reference: the profile formats OASIS expects")
    print("=" * 60)
